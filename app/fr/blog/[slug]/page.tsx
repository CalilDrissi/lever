...
export async function generateStaticParams() {
  const posts = await getAllArticles("fr");
...
export async function generateMetadata({
  ...
  const post = await getArticleBySlug(params.slug, "fr");
  if (!post) return { title: "Article — Virtus Lever" };
  return {
    title: `${post.title} — Virtus Lever`,
    description: post.metaDescription || post.excerpt,
    alternates: { canonical: `/fr/blog/${post.slug}` },
    openGraph: {
      title: post.title,
      description: post.metaDescription || post.excerpt,
      type: "article",
...
export default async function FrBlogPostPage({
  ...
  const [related, headings] = await Promise.all([
    getRelatedForArticle(post, 3, "fr"),
          ...
          <div className="container pt-32 pb-12 sm:pt-36">
            <div className={COLUMN}>
              ...
              <div className="mt-6 flex flex-wrap items-center gap-2">
                {post.tags.map((tag) => (
                  <Link key={tag} href={`/fr/blog/tag/${tagToSlug(tag)}`}>
          ...
          <section className="border-t border-neutral-20 bg-neutral-5">
            <div className="container py-14 sm:py-16">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {related.map((r) => (
                  <PostCard key={r.slug} post={r} basePath="/fr/blog" />
                ))}
              </div>
            </div>
          </section>
