    ...
    <main className="bg-cream text-ink">
      <header className="border-b border-parchment">
        <div className="container py-12">
          <p className="text-eyebrow uppercase text-ink-70 mb-3">
            Virtus Lever — Style Guide
          </p>
          <h1 className="font-display text-h2 tracking-tight">
            Tokens, type, composants.
          </h1>
          <p className="mt-3 text-lead text-ink-70 measure">
            Référence vivante. Chaque élément est annoté avec son token Tailwind
            (<Code>bg-cream</Code>, <Code>text-ink</Code>, <Code>rounded-xl</Code>…)
            pour pouvoir y faire référence pendant les itérations.
          </p>
          <div className="mt-6 flex gap-3">
            <Link href="/" className="text-small text-ink-70 hover:text-ink underline-offset-4 hover:underline">
              ← Retour à la landing
            </Link>
          </div>
        </div>
      ...
      <Section
        n="01"
        title="Couleurs"
        sub="Cream + charcoal + parchment. Le plum est l'unique accent — utilisé une seule fois dans le hero."
      >
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          <Swatch token="bg-cream"        hex="#E9E5DD" use="Fond global page" />
          <Swatch token="bg-cream-50"     hex="#F4F1EB" use="Surface en relief" />
          <Swatch token="bg-cream-100"    hex="#EFEBE3" use="Surface carte" />
          <Swatch token="bg-ink"          hex="#292827" use="Encre principale / sections inversées" inverted />
          <Swatch token="text-ink-70"     hex="#5B5957" use="Type secondaire" />
          <Swatch token="text-ink-50"     hex="#8A8884" use="Hint / muted" />
          <Swatch token="text-ink-30"     hex="#B8B5AE" use="Diviseur appuyé" />
          <Swatch token="border-parchment"      hex="#D9D2C5" use="Bordure par défaut" />
          <Swatch token="border-parchment-dark" hex="#C9C1B1" use="Bordure soulignée" />
          <Swatch token="bg-plum"         hex="#4B3A5A" use="Accent hero (1×)" inverted />
          <Swatch token="bg-plum-ink"     hex="#2E2438" use="Hover bouton primaire" inverted />
          <Swatch token="text-signal"     hex="#3F6B4A" use="Score de levier" />
          <Swatch token="bg-signal-soft"  hex="#E1E8DC" use="Chip score" />
        </div>
      </Section>
      ...
      >
        <div className="space-y-8 border-t border-parchment pt-8">
          <TypeRow
          ...
          <TypeRow
            ...
            className="text-lead text-ink-70"
          />
          <TypeRow
            ...
            className="text-body text-ink-70 measure"
          />
          <TypeRow
            token="text-small / 14px"
            sample="Sans carte bancaire · Gmail & Outlook"
            className="text-small text-ink-70"
          />
          <TypeRow
            token="text-eyebrow / 12px tracking 0.14em uppercase"
            sample="Cockpit de productivité — email"
            className="text-eyebrow uppercase text-ink-50"
          />
        </div>
      </Section>
      ...
      >
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <Card variant="raised">
              <CardHeader>
                <Badge variant="signal">Score 92</Badge>
          ...
          <div>
            ...
            <Card variant="inverted">
              <CardHeader>
                <p className="text-eyebrow uppercase text-cream/60">Sécurité</p>
                <CardTitle className="text-cream">Tes emails restent les tiens.</CardTitle>
              </CardHeader>
              <CardContent className="text-cream/70">
      ...
      <Section
        n="06"
        title="Formulaires"
        sub="Inputs sur fond cream-50, bordure parchment. Focus = ring plum, pas de glow."
      >
        <div className="grid md:grid-cols-2 gap-6 max-w-2xl">
          <div className="space-y-1.5">
            <Label htmlFor="sg-email">Email</Label>
            <Input id="sg-email" placeholder="prenom@domaine.com" />
            <p className="text-small text-ink-50 mt-1">État par défaut — <Code>border-parchment</Code></p>
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="sg-email-focus">Email · focus</Label>
            <Input id="sg-email-focus" placeholder="prenom@domaine.com" autoFocus />
            <p className="text-small text-ink-50 mt-1"><Code>focus:shadow-focus</Code> + <Code>focus:border-plum</Code></p>
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="sg-email-disabled">Email · disabled</Label>
            <Input id="sg-email-disabled" placeholder="prenom@domaine.com" disabled />
            <p className="text-small text-ink-50 mt-1"><Code>disabled:opacity-50</Code></p>
          </div>
          <div className="space-y-1.5">
            <Label>Badges</Label>
            <div className="flex gap-2 flex-wrap">
              ...
              <Badge variant="signal">Score 92</Badge>
              <Badge variant="plum">Domino du jour</Badge>
            </div>
            <p className="text-small text-ink-50 mt-1"><Code>variant=&quot;neutral|signal|plum&quot;</Code></p>
          </div>
        </div>
      </Section>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={token} className="flex items-center gap-4">
              ...
              <div className="flex-1 h-2 rounded-sm bg-parchment relative">
                <div
                  className="absolute inset-y-0 left-0 bg-ink rounded-sm"
                  style={{ width: tokenToWidth(token) }}
                />
              </div>
              <div className="w-44 text-small text-ink-70 text-right">{value}</div>
            </div>
          ))}
        </div>
      </Section>
      ...
      <footer className="border-t border-parchment">
        <div className="container py-10 flex items-center justify-between text-small text-ink-70">
    ...
    <section className="border-b border-parchment">
      <div className="container py-section-sm">
        <div className="flex items-baseline gap-4 mb-2">
          <span className="text-eyebrow uppercase text-ink-50 tabular-nums">{n}</span>
          <h2 className="font-display text-h3 tracking-tight">{title}</h2>
        </div>
        <p className="text-body text-ink-70 measure mb-10">{sub}</p>
        {children}
      </div>
    </section>
  );
...
function Swatch({
  ...
  return (
    <div className="rounded-lg border border-parchment overflow-hidden bg-cream-50">
      <div
      ...
      >
        <span
          ...
          style={{ color: inverted ? "#F4F1EB" : "#292827", opacity: 0.8 }}
        >
          {hex}
        </span>
      </div>
      <div className="p-3">
        <Code>{token}</Code>
        <p className="text-small text-ink-70 mt-1">{use}</p>
      </div>
    </div>
  );
}
...
function RadiusSwatch({ token, radius }: { token: string; radius: string }) {
  return (
    <div className="flex flex-col items-center gap-3">
      <div
        className="w-full h-24 bg-cream-50 border border-parchment-dark"
        style={{ borderRadius: radius }}
      />
      <div className="text-center">
        <Code>{token}</Code>
        <p className="text-small text-ink-70 mt-1">{radius}</p>
      </div>
    </div>
  );
}
...
function ShadowSwatch({ token }: { token: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div className={`w-full h-24 bg-cream-50 rounded-lg ${shadowClass}`} />
      <Code>{token}</Code>
    </div>
  );
}
...
function Code({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <code
      className={`font-mono text-[12px] px-1.5 py-0.5 rounded-sm bg-cream-100 border border-parchment text-ink-70 ${className}`}
    >
      {children}
    </code>
  );
...
function TokenLabel({
  ...
  return (
    <p className={`text-eyebrow uppercase text-ink-50 ${inline ? "" : "mb-2"}`}>
      {children}
    </p>
  );
}
